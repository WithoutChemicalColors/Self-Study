    ...
    while(getline(cin, sent)){
        ...
        while(sent.size()!=0){
            ...
            if( signs.back()=='(' ){
                signs.pop_back();
                vector<int> nums_in;
                vector<char> signs_in; signs_in.push_back('0');
                // ( 1 + 2 + 3 ) + 4
                while( (signs_in.back()!=')') && (sent.size()>0) ){
                    storenums(sent, nums_in);   // store nums_in
                    storesigns(sent, signs_in); // store signs_in
                }
                signs_in.erase(signs_in.begin());
                signs_in.pop_back();
